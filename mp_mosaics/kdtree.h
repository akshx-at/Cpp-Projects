...
class KDTree
{
  ...
  public:
     ...
     */
     // Declration of the helper functions that we used
    KDTree(const vector<Point<Dim>>& newPoints);
    KDTreeNode * Createtree( vector<Point<Dim>>& newPoints,int left, int right, int dimensions);
    //double Distsquared(const Point<Dim>& point1,const Point<Dim>& point2);
    Point<Dim> helperNeighbor(const Point<Dim>& query,int dim,int curRoot);
    KDTreeNode * copytree(KDTreeNode* root);
    void deletetree(KDTreeNode* root);
    Point<Dim> helperNeighbor(const Point<Dim>& query,int dim,KDTreeNode* curRoot) const;
    /**
