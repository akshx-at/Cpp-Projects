...
class SquareMaze
{
  public:
    ...
    private:
    // I create a struct for a each single sqaure which stores two bits, down and right, where the they are either set to true or false where 
    // true means the wall exists and false means that the wall doesnt exist; by default we will set all values of down and right to be true 
    int height_;
    int width_;
    struct square{
      bool down = true;
    ...
    DisjointSets Dset;
      //vector for each square that is used to represent if we can travel or access - make/delete the wall of those squares
      // how do I know how many squares 
      std::vector<std::vector<square>> squarevect; // this create a 2d vector of size  
      std::vector<int> c;
